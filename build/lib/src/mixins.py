...
class ImageCompressorViewMixin:
    compressed_image_fields = None

    def post(self, request, *args, **kwargs):

        if request.method in ('POST', 'PUT') and self.compressed_image_fields:
            ...
            for field in self.compressed_image_fields:
                if request.FILES.get(field):
                    ...
                    if reduce_memory or resize:
                        ...
                        self.compress_image(
                            request, field, resize, temp_folder, **file_details
                        )
    ...
    def compress_image(self, request, field, resize, temp_folder, **kwargs):
        ...
        request.FILES[field] = compressed_image
...
class ImageCompressorAdminMixin:
    """ Mixin for ModelAdmin Class """
